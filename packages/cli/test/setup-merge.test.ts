...
describe('mergeMaps', () => {
  it('keeps positions, renamed names, notes, and all cases from the previous map', () => {
    const prev: AppMap = { version: 1, baseUrl: 'http://a', screens: [screen('home', '/|Home', { x: 100, y: 200, name: 'Landing', notes: 'hero' }), screen('old', '/old|Old')], edges: [],
      cases: [{ id: 'c1', title: 'kept', path: ['home'], steps: ['go to /'], status: 'passed', test: 'e2e/x.yaml' }] };
    const fresh: AppMap = { version: 1, baseUrl: 'http://a', screens: [screen('home', '/|Home', { x: 40, y: 40 }), screen('new', '/new|New', { x: 360, y: 40 })], edges: [], cases: [{ id: 'case-new', title: 'a visitor can reach "New"', path: ['new'], steps: [], status: 'proposed' }] };
    ...
    expect(m.cases.map((c) => c.id)).toEqual(['c1', 'case-new']);
